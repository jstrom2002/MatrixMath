...
FIRFilter::FIRFilter(filterType filt_t, int num_taps, double Fs, double Fx){
	m_error_flag = 0;
	m_filt_t = filt_t;
	m_num_taps = num_taps;
	m_Fs = Fs;
...
// Handles BPF case
FIRFilter::FIRFilter(filterType filt_t, int num_taps, double Fs, double Fl,double Fu){
	m_error_flag = 0;
	m_filt_t = filt_t;
	m_num_taps = num_taps;
	m_Fs = Fs;
...
void FIRFilter::designHPF(){
	int n;
	double mm;

	for (n = 0; n < m_num_taps; n++) {
		mm = n - (m_num_taps - 1.0) / 2.0;
		if (mm == 0.0) m_taps[n] = 1.0 - m_lambda / PI;
		else m_taps[n] = -sin(mm * m_lambda) / (mm * PI);
	}
	return;
}

void FIRFilter::designBPF(){
	int n;
	double mm;
...
void FIRFilter::get_taps(double *taps){
	int i;
...
int FIRFilter::write_taps_to_file(char *filename){
	FILE *fd;
...
#define NP 1000
int
FIRFilter::write_freqres_to_file(char *filename){
	FILE *fd;
	int i, k;
	double w, dw;
...
double FIRFilter::do_sample(double data_sample){
	int i;
	double result;
...
OnePole::OnePole() { a0 = 1.0; b1 = 0.0; z1 = 0.0; };
OnePole::OnePole(double Fc) { z1 = 0.0; setFc(Fc); };
OnePole::~OnePole() {};
