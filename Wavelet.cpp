//code ported from original code by Rafat Hussain
//https://github.com/rafat/wavelib
//see also: http://rafat.github.io/sites/wavebook/index.html
...
void* per_ext2d(std::vector<std::vector<double> > &signal, std::vector<std::vector<double> > &temp2, int a) {
	...
	std::vector<std::vector<double> > temp_vec(rows, std::vector<double>(cols2 + 2 * a));
	//	std::vector<std::vector<double> > temp2(rows + 2 * a ,std::vector<double>(cols + 2* a));
...
void* swt_2d(std::vector<std::vector<double> > &sig, int J, std::string nm, std::vector<double> &swt_output) {
	...
	for (int iter = 0; iter < J; iter++) {
		...
		std::vector<std::vector<double> > signal(rows_n + lf, std::vector<double>(cols_n + lf));
		...
		std::vector<std::vector<double> > sigL(rows_n + lf, std::vector<double>(cols_n));
		std::vector<std::vector<double> > sigH(rows_n + lf, std::vector<double>(cols_n));
		std::vector<std::vector<double> > cA(rows_n, std::vector<double>(cols_n));
		std::vector<std::vector<double> > cH(rows_n, std::vector<double>(cols_n));
		std::vector<std::vector<double> > cV(rows_n, std::vector<double>(cols_n));
		std::vector<std::vector<double> > cD(rows_n, std::vector<double>(cols_n));
...
void* iswt(std::vector<double> &swtop, int J, std::string nm, std::vector<double> &iswt_output) {
	int N = swtop.size() / (J + 1);

	std::vector<double> lpd, hpd, lpr, hpr;
	filtcoef(nm, lpd, hpd, lpr, hpr);

	std::vector<double> appx_sig;

	std::vector<double> low_pass = lpr;
	std::vector<double> high_pass = hpr;
	int lf = low_pass.size();

	for (int iter = 0; iter < J; iter++) {
		std::vector<double> det_sig;
		if (iter == 0) {
			for (int i = 0; i < N; i++) {
				double temp = swtop[i];
...
std::vector<double> swt(std::vector<double> signal1, int J, std::string filterCoefficientName, std::vector<double> swt_output, int length) {
	...
	filtcoef(filterCoefficientName, lpd, hpd, lpr, hpr);
...
void* dwt_output_dim_sym(std::vector<int> &length, std::vector<int> &length2, int J) {
...
void* dispDWT(std::vector<double> &output, std::vector<std::vector<double> > &dwtdisp, std::vector<int> &length, std::vector<int> &length2, int J) {
	int sum = 0;
	...
	for (int iter = 0; iter < J; iter++) {
		...
		std::vector<std::vector<double> >  dwt_output(2 * rows_n, std::vector<double>(2 * cols_n));
		if (iter == 0) {
			for (int i = 0; i < rows_n; i++) {
				for (int j = 0; j < cols_n; j++) {
...
void symm_ext2d(std::vector<std::vector<double> > &signal, std::vector<std::vector<double> > &temp2, int a) {
	unsigned int rows = signal.size();
	unsigned int cols = signal[0].size();
	std::vector<std::vector<double> > temp_vec(rows, std::vector<double>(cols + 2 * a));
	//	std::vector<std::vector<double> > temp2(rows + 2 * a ,std::vector<double>(cols + 2* a));
...
void* circshift2d(std::vector<std::vector<double> > &signal, int x, int y) {
	unsigned int rows = signal.size();
	unsigned int cols = signal[0].size();
	std::vector<std::vector<double> > temp_vec(rows, std::vector<double>(cols));
...
void* idwt_2d_sym(std::vector<double>  &dwtop, std::vector<double> &flag, std::string nm,
	std::vector<std::vector<double> > &idwt_output, std::vector<int> &length) {
	int J = (int)flag[0];
	int rows = length[0];
	int cols = length[1];
	...
	for (int iter = 0; iter < J; iter++) {
		...
		std::vector<std::vector<double> >  cLH(rows_n, std::vector<double>(cols_n));
		std::vector<std::vector<double> >  cHL(rows_n, std::vector<double>(cols_n));
		std::vector<std::vector<double> >  cHH(rows_n, std::vector<double>(cols_n));
		...
		std::vector<std::vector<double> > cL(2 * len_x - lf + 2, std::vector<double>(len_y));
		std::vector<std::vector<double> > cH(2 * len_x - lf + 2, std::vector<double>(len_y));
		...
		if (iter == 0) {
			...
			for (unsigned int j = 0; j < cols1;j++) {
				std::vector<double> temp_L1, temp_L2, oup;
		...
		std::vector<std::vector<double> > signal(2 * len_x - lf + 2, std::vector<double>(2 * len_y - lf + 2));
...
void* dwt2_sym(std::string name, std::vector<std::vector<double> > &signal, std::vector<std::vector<double> >  &cLL,
	std::vector<std::vector<double> >  &cLH, std::vector<std::vector<double> >  &cHL, std::vector<std::vector<double> > &cHH) {
	//Analysis
	...
	std::vector<std::vector<double> > lp_dn1(rows, std::vector<double>(cols_lp1));
	std::vector<std::vector<double> > hp_dn1(rows, std::vector<double>(cols_hp1));

	// Implementing row filtering and column downsampling in each branch.
	for (int i = 0; i < rows; i++) {
		std::vector<double> temp_row, oup_lp, oup_hp;
	...
	for (int j = 0; j < cols; j++) {
		std::vector<double> temp_row3, oup_lp, oup_hp;
		for (int i = 0;i < rows;i++) {
	...
	for (int j = 0; j < cols; j++) {
		std::vector<double> temp_row5, oup_lp, oup_hp;
		for (int i = 0;i < rows;i++) {
...
void* dwt_2d_sym(std::vector<std::vector<double> > &origsig, int J, std::string nm, std::vector<double> &dwt_output
	...
	std::vector<std::vector<double> > original_copy(rows_n, std::vector<double>(cols_n));

	original_copy = sig;
	int Max_Iter;
	int temp1 = (int)ceil(log(double(sig.size())) / log(2.0));
	int temp2 = (int)ceil(log(double(sig[0].size())) / log(2.0));
	Max_Iter = (temp1 > temp2) ? temp1 : temp2;
	if (Max_Iter < J) {
...
void* idwt1_sym(std::string wname, std::vector<double> &X, std::vector<double> &app, std::vector<double> &detail) {

	// Not Tested. Use dwt_sym and idwt_sym for any and all computations
	std::vector<double> dwtop;
...
void* idwt1_sym_m(std::string wname, std::vector<double> &idwt_output, std::vector<double> &app, std::vector<double> &detail) {
...
void* idwt_sym(std::vector<double> &dwtop, std::vector<double> &flag, std::string nm,
...
void* dwt1_sym(std::string wname, std::vector<double> &signal, std::vector<double> &cA, std::vector<double> &cD) {
...
void* dwt1_sym_m(std::string wname, std::vector<double> &signal, std::vector<double> &cA, std::vector<double> &cD) {
...
void* dwt_sym(std::vector<double> &signal, int J, std::string nm, std::vector<double> &dwt_output,
	std::vector<double> &flag, std::vector<int> &length) {
	...
	std::vector<double> original_copy, appx_sig, det_sig;
...
void* freq(std::vector<double> &sig, std::vector<double> &freq_resp) {
	unsigned int K = sig.size();
	unsigned int N = (unsigned int)pow(2.0, ceil(log10(static_cast<double>(K)) / log10(2.0)));
	std::vector<std::complex<double> > fft_oup;
	for (unsigned int i = 0; i < sig.size(); i++) {
		double temp = sig[i];
		fft_oup.push_back(std::complex<double>(temp, 0));
	}
	fft(fft_oup, 1, N);
...
double convfft(std::vector<double> &a, std::vector<double> &b, std::vector<double> &cx) {
...
double convfftm(std::vector<double> &a, std::vector<double> &b, std::vector<double> &cx) {
...
void* fft(std::vector<std::complex<double>> &data, int sign, unsigned  int N) {
...
void* dwt(std::vector<double> &sig, int J, std::string nm, std::vector<double> &dwt_output
	, std::vector<double> &flag, std::vector<int> &length) {
	...
	std::vector<double> original_copy, orig, appx_sig, det_sig;
	original_copy = sig;
...
void circshift(std::vector<double> &sig_cir, int L) {
...
void downsamp(std::vector<double> &sig, int M, std::vector<double> &sig_d) {
	int len = sig.size();
...
void* dwt1(std::string wname, std::vector<double> &signal, std::vector<double> &cA, std::vector<double> &cD) {
...
void* idwt1(std::string wname, std::vector<double> &X, std::vector<double> &cA, std::vector<double> &cD) {
	std::vector<double> lpd1, hpd1, lpr1, hpr1;
...
void upsamp(std::vector<double> &sig, int M, std::vector<double> &sig_u) {
	int len = sig.size();
	double len_n = ceil((double)len * (double)M);
...
void* getcoeff2d(std::vector<std::vector<double> > &dwtoutput, std::vector<std::vector<double> > &cH,
	std::vector<std::vector<double> > &cV, std::vector<std::vector<double> > &cD, std::vector<double> &flag, int &N) {
	if (N > flag[2]) {
...
void* zero_remove(std::vector<std::vector<double> > &input, std::vector<std::vector<double> > &output) {
	...
	std::vector<std::vector<double> >::iterator row = output.end() - zero_rows;
	...
	for (unsigned int i = 0; i < ousize; i++) {
		std::vector<double> ::iterator col = output[i].end() - zero_cols;
...
void* idwt_2d(std::vector<double>  &dwtop, std::vector<double> &flag, std::string nm,
	std::vector<std::vector<double> > &idwt_output, std::vector<int> &length) {
	int J = (int)flag[0];
	int rows = length[0];
	int cols = length[1];
	...
	for (int iter = 0; iter < J; iter++) {
		...
		// Row Upsampling and Column Filtering at the first LP Stage
		std::vector<std::vector<double> > cL(2 * len_x, std::vector<double>(len_y));
		std::vector<std::vector<double> > cH(2 * len_x, std::vector<double>(len_y));
		...
		std::vector<std::vector<double> > signal(2 * len_x, std::vector<double>(2 * len_y));
...
void* dwt_2d(std::vector<std::vector<double> > &origsig, int J, std::string nm, std::vector<double> &dwt_output
	, std::vector<double> &flag, std::vector<int> &length) {
	// flag will contain
...
void* branch_lp_hp_up(std::string wname, std::vector<double> &cA, std::vector<double> &cD, std::vector<double> &X) {
	std::vector<double> lpd1, hpd1, lpr1, hpr1;
...
void* branch_hp_dn(std::string wname, std::vector<double> &signal, std::vector<double> &sigop) {
...
void* branch_lp_dn(std::string wname, std::vector<double> &signal, std::vector<double> &sigop) {
...
void* idwt2(std::string name, std::vector<std::vector<double> > &signal, std::vector<std::vector<double> >  &cLL,
	std::vector<std::vector<double> >  &cLH, std::vector<std::vector<double> >  &cHL, std::vector<std::vector<double> > &cHH) {
	// Synthesis
	int rows = cLL.size();
	int cols = cLL[0].size();
	int rows_n = 2 * rows;
	// Row Upsampling and Column Filtering at the first LP Stage
	std::vector<std::vector<double> > cL(rows_n, std::vector<double>(cols));
	std::vector<std::vector<double> > cH(rows_n, std::vector<double>(cols));
...
void* dwt2(std::string name, std::vector<std::vector<double> > &signal, std::vector<std::vector<double> >  &cLL,
	std::vector<std::vector<double> >  &cLH, std::vector<std::vector<double> >  &cHL, std::vector<std::vector<double> > &cHH) {
	//Analysis
	int rows = signal.size();
	int cols = signal[0].size();
...
void* downsamp2(std::vector<std::vector<double> > & vec1, std::vector<std::vector<double> > & vec2, int rows_dn, int cols_dn) {
...
void* upsamp2(std::vector<std::vector<double> > & vec1, std::vector<std::vector<double> > & vec2, int rows_up, int cols_up) {
...
int filtcoef(std::string name, std::vector<double> &lp1, std::vector<double> &hp1, std::vector<double> &lp2,
	std::vector<double> &hp2) {
